...
class Program
{
    ...
    private static void CustomizeSchema(JsonSchema schema)
    {
        ...
        if (schema.Properties.TryGetValue("Overlay", out var overlayProperty))
        {
            ...
            if (overlaySchema.Properties.TryGetValue("ChromeHandling", out var chromeHandlingProp))
            {
                chromeHandlingProp.Description = "Linux only. Whether the desktop shell's own surfaces - notifications, on-screen displays, panel menus, the panel and the dock - are dimmed with your windows. 'Highlight' (default) keeps them lit, 'Dim' covers them like any other window. Ignored by the Windows client.";
            }

            if (overlaySchema.Properties.TryGetValue("AlwaysOnTopHandling", out var alwaysOnTopHandlingProp))
            {
                alwaysOnTopHandlingProp.Description = "Linux only. What happens to windows pinned always-on-top: 'Ignore' (default) dims them like any other window, 'Highlight' treats them as part of the spotlight, 'Dim' covers them with the inactive overlay. Ignored by the Windows client.";
            }
        }
