...
4. **Moves [Unreleased] to [X.Y.Z] in CHANGELOG.md** using `Move-UnreleasedToVersion.ps1`
...
10. Creates a git tag with the new patch version
...
4. **Move [Unreleased] to versioned section**: Run `pwsh SpotlightDimmer.Scripts/Move-UnreleasedToVersion.ps1 -Version X.Y.(Z+1)`
...
7. **If any validation step fails**:
   ...
   - Revert CHANGELOG.md changes (move [X.Y.Z] back to [Unreleased])
