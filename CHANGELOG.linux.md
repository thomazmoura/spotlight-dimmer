...
- **Neovim split spotlight**: When the focused tmux pane runs neovim, the spotlight can now narrow down to the focused neovim split. The other splits are dimmed with the same overlay as inactive panes and windows, so plugins like `tint.nvim` are no longer needed and the dimming matches your configured opacity
  ...
  - While a command (`:`) or a prompt is being typed, such as nvim-tree's "create file", the spotlight moves to the command line: the bottom rows, or noice.nvim's floating `cmdline_popup` wherever it is placed, border included. Before, the prompt could sit dimmed over another split. Searches (`/`, `?`) light the whole pane, so the matches in the buffer stay visible
...
- **Spotlight em splits do neovim**: Quando o painel do tmux em foco executa o neovim, o spotlight agora pode se restringir ao split do neovim em foco. Os demais splits são escurecidos com a mesma sobreposição usada para painéis e janelas inativos, então plugins como o `tint.nvim` deixam de ser necessários e o escurecimento segue a opacidade configurada
  ...
  - Enquanto um comando (`:`) ou um prompt é digitado, como o "criar arquivo" do nvim-tree, o spotlight passa para a linha de comando: as linhas inferiores, ou o `cmdline_popup` flutuante do noice.nvim onde quer que esteja, incluindo a borda. Antes, o prompt podia ficar escurecido sobre outro split. Buscas (`/`, `?`) destacam o painel inteiro, para que as ocorrências no buffer continuem visíveis
