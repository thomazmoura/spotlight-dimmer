...
- **Neovim split spotlight**: When the focused tmux pane runs neovim, the spotlight can now narrow down to the focused neovim split. The other splits are dimmed with the same overlay as inactive panes and windows, so plugins like `tint.nvim` are no longer needed and the dimming matches your configured opacity
  ...
  - Floating windows (Telescope, pickers), the command line (`:`, `/`, and prompts like nvim-tree's "create file", including noice's floating `cmdline_popup`, which would otherwise sit dimmed over another split), tmux copy-mode and `choose-tree`, and a suspended or exited neovim all fall back to highlighting the whole tmux pane
  ...
  - If other code already sets `titlestring` on the host, pass `manage_title = false` to `setup()` and append `require("spotlight-dimmer").title_segment()` to your own title
...
- **Spotlight em splits do neovim**: Quando o painel do tmux em foco executa o neovim, o spotlight agora pode se restringir ao split do neovim em foco. Os demais splits são escurecidos com a mesma sobreposição usada para painéis e janelas inativos, então plugins como o `tint.nvim` deixam de ser necessários e o escurecimento segue a opacidade configurada
  ...
  - Janelas flutuantes (Telescope, seletores), a linha de comando (`:`, `/` e prompts como o "criar arquivo" do nvim-tree, incluindo o `cmdline_popup` flutuante do noice, que de outra forma ficaria escurecido sobre outro split), o copy-mode e o `choose-tree` do tmux, e um neovim suspenso ou encerrado voltam a destacar o painel inteiro do tmux
  ...
  - Se outro código já define o `titlestring` no host, passe `manage_title = false` para o `setup()` e acrescente `require("spotlight-dimmer").title_segment()` ao seu próprio título
