...
Function CreateDesktopShortcut
  ; Explicitly specify icon file from resources directory
  ; CreateShortcut syntax: link target parameters iconfile iconindex startoptions keyboard description
  CreateShortcut "$DESKTOP\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "$INSTDIR\spotlight-dimmer-icon.ico" 0
...
Function CreateStartMenuShortcut
  ...
  ; Explicitly specify icon file from resources directory
  ; CreateShortcut syntax: link target parameters iconfile iconindex startoptions keyboard description
  CreateShortcut "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "$INSTDIR\spotlight-dimmer-icon.ico" 0
