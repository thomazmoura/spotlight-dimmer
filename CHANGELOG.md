...
- **Novas configurações de escurecimento do Linux aceitas pelo schema**: O `Overlay.ChromeHandling` e o `Overlay.AlwaysOnTopHandling`, introduzidos pelo daemon Linux, faziam o VS Code marcar um `config.json` perfeitamente válido como inválido para quem compartilha o mesmo arquivo entre Windows e Linux. As duas chaves agora fazem parte do `config.schema.json`, documentadas como exclusivas do Linux e ignoradas pelo cliente Windows
...
- **tmux pane border lines left dimmed**: The spotlight over a focused tmux pane stopped at the pane's interior, leaving the border characters around it dimmed like a dark padding. The highlight now extends one cell over every adjacent pane border, so the lines framing the focused pane are highlighted with it (edges touching the terminal or status bar are unaffected)
...
- **Linhas de borda do painel tmux deixadas escurecidas**: O spotlight sobre o painel tmux em foco parava no interior do painel, deixando os caracteres de borda ao seu redor escurecidos como um preenchimento escuro. O destaque agora se estende uma célula sobre cada borda de painel adjacente, então as linhas que emolduram o painel em foco são destacadas junto com ele (bordas que tocam o terminal ou a barra de status não são afetadas)
